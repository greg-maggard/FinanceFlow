...
public func progressOf(_ state: AppState, _ id: NodeId) -> ProgressInfo {
    ...
    if recurringNodes.contains(id) {
        ...
        if let items = data?.items, !items.isEmpty {
            ...
            if target > 0 {
                return .goal(value: funded, max: target, ready: funded >= target)
            }
    ...
    case .SmallEF:
        ...
        return .goal(value: balance, max: target, ready: balance >= target)
    case .BigEF:
        ...
        return .goal(value: balance, max: target > 0 ? target : 1, ready: target > 0 && balance >= target)
    case .Match:
        let matchPct = data?.match?.matchPct ?? 0
        let cur = data?.match?.currentContribPct ?? 0
        return .goal(value: cur, max: matchPct > 0 ? matchPct : 1, ready: matchPct > 0 && cur >= matchPct)
    case .IRA:
        let ytd = data?.ira?.ytdContribution.value ?? 0
        let limit = data?.ira?.annualLimit ?? state.settings.iraAnnualLimit
        return .goal(value: ytd, max: limit, ready: ytd >= limit)
    case .HSA:
    ...
    case .Increase401k:
        ...
        return .goal(value: cur, max: target, ready: cur >= target)
    case .SavePurchase:
        let saved = data?.savePurchase?.saved.value ?? 0
        let target = data?.savePurchase?.target ?? 0
        return .goal(value: saved, max: target > 0 ? target : 1, ready: target > 0 && saved >= target)
